...
library(vcfR)
if(file.exists(paste0("$PATH-TO-SERVER/A_thaliana/temp/temp/ancestry447/",argum[2],".files/",argum[1],".recode.vcf"))) {
  
  vcf<-read.vcfR(paste0("$PATH-TO-SERVER/A_thaliana/temp/temp/ancestry447/",argum[2],".files/",argum[1],".recode.vcf"))
...
if(file.exists(paste0("$PATH-TO-SERVER/A_thaliana/temp/temp/ancestry447/",argum[2],".files/",argum[1],".gff"))) {
  
  gff<-read.table(paste0("$PATH-TO-SERVER/A_thaliana/temp/temp/ancestry447/",argum[2],".files/",argum[1],".gff"))
  ...
  fasta<-read.FASTA(paste0("$PATH-TO-SERVER/A_thaliana/temp/temp/ancestry447/",argum[2] ,".files/","mafft_",argum[1]))
...
# remove snp if none of the outgroup has an allele
  ...
  resultall<-read.table(paste0("$PATH-TO-SERVER/A_thaliana/temp/temp/ancestry447/",argum[2] ,".files/","data-file.",argum[2] ,".txt"),h=T,sep="\t")
  resultall<-rbind(resultall,result)
  
  snpslistall<-read.table(paste0("$PATH-TO-SERVER/A_thaliana/temp/temp/ancestry447/",argum[2] ,".files/","data-file.snps.",argum[2] ,".txt"),h=T,sep="\t")
  snpslistall<-rbind(snpslistall,snpslist)
  
  write.table(x = resultall,file = paste0("$PATH-TO-SERVER/A_thaliana/temp/temp/ancestry447/",argum[2] ,".files/","data-file.",argum[2] ,".txt"),sep = "\t",quote = F,row.names = F,col.names = T)
  write.table(x = snpslistall,file = paste0("$PATH-TO-SERVER/A_thaliana/temp/temp/ancestry447/",argum[2] ,".files/","data-file.snps.",argum[2] ,".txt"),sep = "\t",quote = F,row.names = F,col.names = T)
