...
print(paste0(i,"/5"))
  if (i==1) {
    chrm<-read.table(paste0("$PATH-TO-SERVER/A_thaliana/fastphase/scanEHH.chrm-",i,".txt"),h=T,sep=" ",dec=".")
  }else{
    chrm<-rbind(chrm,read.table(paste0("$PATH-TO-SERVER/A_thaliana/fastphase/scanEHH.chrm-",i,".txt"),h=T,sep=" ",dec="."))
  }
}
...
#load AD data
print("load AD data")
#genelist.f<-read.table("$PATH-TO-SERVER/A_thaliana/temp/temp/ancestry447/gene_lists.txt")[,1] 
genelist<-substr(grep("output-file-pvalues",list.files("$PATH-TO-SERVER/software/est_sfs/"),value = T),21,32)
...
for (i in 1:length(genelist)) {
 print(paste0(i,"/",length(genelist)))
  estsfs<-read.table(paste0("$PATH-TO-SERVER/software/est_sfs/output-file-pvalues.",genelist[i],".txt"),skip=8)[,3]
  estprob<-read.table(paste0("$PATH-TO-SERVER/software/est_sfs/data-file.",genelist[i],".txt"),h=F)
  estsnp<-read.table(paste0("$PATH-TO-SERVER/A_thaliana/temp/temp/ancestry447/",genelist[i],".files/data-file.snps.",genelist[i],".txt"),h=T)
