#!/usr/bin/env Rscript
piece = commandArgs(trailingOnly=TRUE)
...
rm(list = ls()[-which(ls()=="piece")])

library(vcfR,lib.loc="/domus/h1/kevinfrs/R/x86_64-pc-linux-gnu-library/")
library(RGenetics,lib.loc="/domus/h1/kevinfrs/R/x86_64-pc-linux-gnu-library/4.2")
library(pegas,lib.loc="/domus/h1/kevinfrs/R/x86_64-pc-linux-gnu-library/")
...
result<-read.table(paste0("../result_",piece,".txt"),h=T,sep=" ")
result[dim(result)[1]+1,]<-c(names(fasta),nchar(fasta),length(ffold),length(zfold),pis,pin,pinpis)
write.table(x = result,file = paste0("../result_",piece,".txt"),quote = F,row.names = F,col.names = T)
