...
#arguments<-c("AT1G77090","HS01")
###################
#
# Input: Standard files gff, fasta, vcf
# output: PiN/PiS
...
result<-read.table(paste0("result_",arguments[1],".txt"),h=T,sep=" ")

# leave 20% out error
nb_acc<-length(acc)/2
nb_sample<-round(nb_acc*.8,0)
set.seed(123)
for (i in 1:100) {
  s<-sample(x = 1:nb_acc,size = nb_sample,replace = F)
  s2<-c(s,s+nb_acc)
  
  pin<-nuc.div(zfasta[s2])
  pis<-nuc.div(ffasta[s2])
  pinpis<-pin/pis
  result[dim(result)[1]+1,]<-c(arguments[2],i,pis,pin,pinpis)
  }
...
#nb_acc<-length(acc)/2
#for (i in 1:nb_acc) {
#  pin<-nuc.div(zfasta[-c(i,i+nb_acc)])
#  pis<-nuc.div(ffasta[-c(i,i+nb_acc)])
#  pinpis<-pin/pis
#  finalpinpis<-c(finalpinpis,pinpis)
#}

# write results
write.table(x = result,file = paste0("result_",arguments[1],".txt"),quote = F,row.names = F,col.names = T)
